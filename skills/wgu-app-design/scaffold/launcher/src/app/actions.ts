...
const VIEW_AS_COOKIE = 'launcher_view_as'

export async function adminStartViewAs(email: string) {
  const user = await getCurrentUser()
  if (!user) throw new Error('Not signed in')
  // Use the underlying admin flag — if you're already impersonating,
  // your isAdmin is the target's. Check the impersonator field to detect
  // and use the real admin's status.
 ...
 * role's defaults: add any missing grants, remove any extras. The
 * checkbox grid in the admin UI reflects the result so the admin can
 * still customize on top. Hidden apps are never auto-granted —
 * they remain admin-grant only.
 */
export async function adminSetRole(email: string, role: MemberRole | null) {
  ...
  if (role === null) {
    // Just persist the cleared role; leave grants untouched.
    return
  }
  ...
  // Adds: desired but not currently granted.
  for (const slug of desired) {
    if (!existingSlugs.has(slug as AppSlug)) {
      // Defensive: only call if slug is a real AppSlug.
  ...
  // Removes: currently granted but not desired by role.
  // Skip hidden apps (admin granted these explicitly; role defaults
  // never include them so leaving them alone is the right call).
  for (const grant of existing) {
    if (desired.has(grant.appSlug)) continue
    // Look up the app to check if it's hidden.
...
export async function adminSetIsAdmin(email: string, isAdmin: boolean) {
  const admin = await requireAdmin()
  // Self-revoke guard: only block when the caller would actually lose
  // admin. Hard-coded admins keep access via ADMIN_EMAILS regardless of
  // the Smartsheet flag, so let them toggle their own row freely.
  if (
...
//
// For each known source app, query its roster sheet, find rows whose
// email matches a master roster member with no avatar yet, and copy
// the URL across.
//
// Replace the AVATAR_SOURCES entries below with your own app sheets.
// =============================================================================
...
const AVATAR_SOURCES: Array<{
  ...
  // Replace these with your own avatar source apps.
  // { app: 'example-tool', sheetId: 0, emailCol: 0, avatarCol: 0 },
]
...
//
// Drives the /settings/profile page. Backed by the master Members
// Smartsheet so a change here immediately propagates to all apps in
// the launcher (each app reads the same sheet on render).
// =============================================================================
